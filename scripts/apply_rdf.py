...
arg6: the target segmentation file
"""
...
def main():
    ...
    probability_results = []
    for _fv in numpy.array_split(feature_vector, 20):
        probability_results.append(forest.predict_proba(_fv))
    probability_results = numpy.vstack(probability_results)
    classification_results = numpy.argmax(probability_results, -1)

    # preparing  image
    m, h = load(mask_file)
    m = m.astype(numpy.bool)
    oc = numpy.zeros(m.shape, numpy.uint8)
    op = numpy.zeros(m.shape + (probability_results.shape[-1], ), numpy.float32)
    oc[m] = numpy.squeeze(classification_results).ravel()
    op[m] = numpy.squeeze(probability_results).reshape(numpy.prod(probability_results.shape[:-1]), probability_results.shape[-1])
